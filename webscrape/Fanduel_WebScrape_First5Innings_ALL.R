###############################################
# Scraping first 5 innings from TeamRanking   #
###############################################
...
# Fixing the links for year

## 2017
df_2017 <- df %>% 
  mutate(url = paste0('https://www.teamrankings.com', stat_links, '?date=2017-11-02'))
df1_2017 <- df1 %>% 
  mutate(url = paste0('https://www.teamrankings.com', stat_links, '?date=2017-11-02'))

## 2018
df_2018 <- df %>% 
  mutate(url = paste0('https://www.teamrankings.com', stat_links, '?date=2018-10-29'))
df1_2018 <- df1 %>% 
  mutate(url = paste0('https://www.teamrankings.com', stat_links, '?date=2018-10-29'))

## 2019
df_2019 <- df %>% 
  mutate(url = paste0('https://www.teamrankings.com', stat_links, '?date=2019-10-31'))
df1_2019 <- df1 %>% 
  mutate(url = paste0('https://www.teamrankings.com', stat_links, '?date=2019-10-31'))

## 2020
df_2020 <- df %>% 
  mutate(url = paste0('https://www.teamrankings.com', stat_links, '?date=2020-10-28'))
df1_2020 <- df1 %>% 
  mutate(url = paste0('https://www.teamrankings.com', stat_links, '?date=2020-10-28'))
